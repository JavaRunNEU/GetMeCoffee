...
extension DHCollectionTableViewController:UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        cell.contentView.addSubview(UIView(frame: CGRectMake(0, 0, 100, 100)))
...
//        self.navigationController?.pushViewController(vc, animated: true)
        if UIDevice.currentDevice().systemVersion >= "8.0" {
            let alert = UIAlertController(title: "The CV tag \(collectionView.tag)行", message: "item index path\(indexPath.item)", preferredStyle: UIAlertControllerStyle.Alert)
